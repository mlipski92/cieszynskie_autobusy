<div class="p-6 bg-white rounded-xl shadow-md">
    <h2 class="text-2xl font-bold mb-4 text-gray-800">Lista linii</h2>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($lineList as $line)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-2 text-sm text-gray-700">{{ $line->id }}</td>
                        <td class="px-4 py-2 text-sm text-gray-700">{{ $line->name }}</td>
                        <td class="px-4 py-2 text-sm text-gray-700">{{ $line->direction }}</td>
                        <td class="px-4 py-2 text-sm text-gray-600">{{ $line->created_at->format('Y-m-d H:i') }}</td>
                        <td class="px-4 py-2 text-center">
                            <div class="flex gap-[10px] justify-center">
                                ...
                                <a href="{{ route('line.edit', $line->id) }}" class="text-blue-600 hover:underline block">
    ...
    <a href="{{ route("line.create") }}" class="text-black">Dodaj nową linię</a>
</div>
