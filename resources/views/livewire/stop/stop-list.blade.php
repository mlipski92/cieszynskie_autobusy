<div class="p-6 bg-white rounded-xl shadow-md">
    <h2 class="text-2xl font-bold mb-4 text-gray-800">Lista przystanków</h2>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($stopList as $stop)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-2 text-sm text-gray-700">{{ $stop->id }}</td>
                        <td class="px-4 py-2 text-sm text-gray-700">{{ $stop->name }}</td>
                        <td class="px-4 py-2 text-sm text-gray-600">{{ $stop->created_at->format('Y-m-d H:i') }}</td>
                        <td class="px-4 py-2 text-center">
                            <div class="flex gap-[10px] justify-center">
                                @livewire('delete-button', [
                                    'resource' => 'stop',
                                    'model' => App\Models\Stop::class,
                                    'id' => $stop->id,
                                    'returnRoute' => 'stop.list'
                                ])
                                <a href="{{ route('stop.edit', $stop->id) }}" class="text-blue-600 hover:underline block">
    ...
    <a href="{{  route("stop.create") }}" class="text-black">Dodaj nowy przystanek</a>
</div>
